...
def get_history(type: str, value: float, current_balance: float, timestamp: float):
    history_deposit=repo_transacao
    history_withdraw=repo_user
    all_transactions=all_transactions

    return{
        all_transactions:{
            "history_deposit": [history_deposit.to_dict() for Transacao in Transacao ],
            "history_withdraw":[history_withdraw.to_dict() for Transacao in Transacao ]
        },
